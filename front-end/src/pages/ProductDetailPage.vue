...
export default {
  ...
  methods:{
    async addToCart(){
      ...
      alert("Successufully added item to cart!");
    },
    async signIn(){
      const email = prompt('Please enter your email to sign in:');
      const auth = getAuth();
      const actionCodeSettings = {
        url: `https://vue-linkedin-course.onrender.com/api/products/${this.$route.params.productId}`,
        handleCodeInApp: true,
      }
      await sendSignInLinkToEmail(auth, email, actionCodeSettings);
      alert("A login link was sent to email you provided");
